...
const normalizeSlidePhraseVersionKeys = (
  ...
  return rows.map((row: BibleSlideRowState): BibleSlideRowState => {
    ...
    const found: BibleVersionOption | undefined = opts.find(
      (o: BibleVersionOption): boolean => o.label === row.version
    );
    if (found !== undefined) {
...
export const BiblePartEditForm = forwardRef<HTMLElement, BiblePartEditFormProps>(function BiblePartEditForm(
    ...
    (rowId: string, draftRaw: string): void => {
        ...
        setSlideRows((prev: BibleSlideRowState[]): BibleSlideRowState[] =>
          prev.map(
            (r: BibleSlideRowState): BibleSlideRowState =>
              r.id === rowId && r.kind === "phrase"
                ? { ...r, version: "", book: "", chapter: "", verse: "" }
                : r
          )
        );
        setComboDrafts(
      ...
      setSlideRows((prev: BibleSlideRowState[]): BibleSlideRowState[] =>
        prev.map(
          (r: BibleSlideRowState): BibleSlideRowState =>
            r.id === rowId && r.kind === "phrase"
              ? { ...r, version: resolved, book: "", chapter: "", verse: "" }
              : r
        )
      );
      setComboDrafts(
    ...
    (rowId: string, vo: BibleVersionOption): void => {
      ...
      setSlideRows((prev: BibleSlideRowState[]): BibleSlideRowState[] =>
        prev.map(
          (r: BibleSlideRowState): BibleSlideRowState =>
            r.id === rowId && r.kind === "phrase"
              ? { ...r, version: vo.key, book: "", chapter: "", verse: "" }
              : r
        )
      );
      setComboDrafts(
    ...
    (rowId: string, bookCanonical: string): void => {
      ...
      setSlideRows((prev: BibleSlideRowState[]): BibleSlideRowState[] =>
        prev.map(
          (r: BibleSlideRowState): BibleSlideRowState =>
            r.id === rowId && r.kind === "phrase"
              ? { ...r, book: bookCanonical, chapter: "", verse: "" }
              : r
        )
      );
      setComboDrafts(
  ...
  return (
    <aside
      ref={ref}
      className="relative z-20 flex max-h-[45vh] w-full shrink-0 flex-col overflow-hidden border-t border-neutral-200/90 bg-white dark:border-white/[0.08] dark:bg-[#1c1c1e] sm:h-full sm:max-h-none sm:w-[min(100%,18rem)] sm:rounded-2xl sm:border-l sm:border-t-0 sm:border-black/[0.06] sm:dark:border-white/[0.08]"
          ...
          <div className="flex min-w-0 flex-col gap-4">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-col gap-4">
                      ...
                      <Fragment key={row.id}>
                        <div className="rounded-xl border border-dashed border-black/[0.12] bg-white/60 px-3 py-2.5 dark:border-white/[0.12] dark:bg-white/[0.03]">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button
                              type="button"
                              className="shrink-0 rounded-md border border-black/[0.1] bg-white px-2 py-0.5 text-[10px] font-medium text-neutral-600 outline-none transition hover:bg-neutral-50 focus-visible:ring-2 focus-visible:ring-[#0071e3] dark:border-white/[0.12] dark:bg-[#2c2c2e] dark:text-neutral-300 dark:hover:bg-white/5 dark:focus-visible:ring-[#0a84ff]"
                              onClick={(): void => {
                        ...
                        {showTitleInsert && canInsertTitleAfterRow(slideRows, index) ? (
                          <button
                            type="button"
                            className="self-start rounded-md border border-black/[0.06] bg-transparent px-2 py-1 text-[10px] font-medium text-neutral-600 outline-none transition hover:bg-neutral-100 focus-visible:ring-2 focus-visible:ring-[#0071e3] dark:border-white/[0.08] dark:text-neutral-300 dark:hover:bg-white/5 dark:focus-visible:ring-[#0a84ff]"
                            onClick={(): void => {
                  ...
                  if (showCollapsed) {
                    return (
                      <Fragment key={row.id}>
                        <button
                          type="button"
                          className="w-full rounded-lg border border-black/[0.1] bg-white px-2.5 py-2 text-left text-[12px] text-neutral-800 outline-none transition hover:bg-neutral-50 focus-visible:ring-2 focus-visible:ring-[#0071e3] dark:border-white/[0.12] dark:bg-[#2c2c2e] dark:text-neutral-100 dark:hover:bg-white/5 dark:focus-visible:ring-[#0a84ff]"
                          onClick={(): void => {
                        ...
                        >
                          <span className="block truncate text-neutral-800 dark:text-neutral-100">{phrasePreviewText}</span>
                    ...
                    <Fragment key={row.id}>
                      ...
                      <div className="-mx-3 -mt-3 flex min-h-7 items-stretch gap-2 px-3 pt-2">
                        {canFold ? (
                          <button
                            type="button"
                            className="min-w-0 flex-1 border-0 bg-transparent px-0 py-0 text-left text-[11px] font-semibold text-neutral-700 shadow-none outline-none ring-0 hover:bg-transparent focus:bg-transparent focus:outline-none focus:ring-0 focus-visible:bg-transparent focus-visible:outline-none focus-visible:ring-0 active:bg-transparent disabled:opacity-50 dark:text-neutral-200"
                            onClick={(): void => {
                        ...
                        <button
                          type="button"
                          className="shrink-0 self-center rounded-md border border-black/[0.1] bg-white px-2 py-0.5 text-[10px] font-medium text-neutral-600 outline-none transition hover:bg-neutral-50 focus-visible:ring-2 focus-visible:ring-[#0071e3] dark:border-white/[0.12] dark:bg-[#2c2c2e] dark:text-neutral-300 dark:hover:bg-white/5 dark:focus-visible:ring-[#0a84ff]"
                          onClick={(): void => {
                            handleRemoveSlideRow(row.id);
                          }}
                          disabled={isSaving}
                ...
                <button
                  type="button"
                  className="self-start rounded-md border border-black/[0.1] bg-white px-2.5 py-1 text-[11px] font-medium text-neutral-800 outline-none transition hover:bg-neutral-50 focus-visible:ring-2 focus-visible:ring-[#0071e3] disabled:cursor-not-allowed disabled:opacity-50 dark:border-white/[0.12] dark:bg-[#2c2c2e] dark:text-neutral-100 dark:hover:bg-white/5 dark:focus-visible:ring-[#0a84ff]"
